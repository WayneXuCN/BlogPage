...
- NumPy 通过 Apple 的 Accelerate 框架（BLAS）进行了优化，使用 conda 时，需要指定 `numpy` 对 BLAS 库的依赖关系。如果在 PyPI 中通过 Wheel 安装，`numpy>2.0`将自动使用 Accelerate 框架的 BLAS。
